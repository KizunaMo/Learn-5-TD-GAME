...
public class Node : MonoBehaviour
{
    ...
    private void OnMouseDown()//按下滑鼠按鍵時
    {
        ...
        if (buildManager.GetTurretToBuild() == null)//當函式為空時(未點擊任何對象時，函式式空的)
            return;
        ...
        //bulid a turret
        GameObject turretToBuild = BuildManager.instacnce.GetTurretToBuild();//當shop使用SetTurretToBuild時，新增滑鼠點擊對象傳值進去GetTurretToBuild()的turretToBuild,再用GetTurretToBuild把值直接return
        ...
        //}
        turret = (GameObject)Instantiate(turretToBuild, transform.position + positionOffset, transform.rotation);//新增一個GameObject,turret等於GetTurretToBuild()的turretToBuild
